...
def test_task_modification() -> None:
    ...
    due_date = date.today() + timedelta(days=1)
    assignee = "Edouard"
    task = Task(1, "Dish", "Wash the dishes after dinner", due_date,
                assignee, TaskStatus.IN_PROGRESS, TaskPriority.LOW, "Edouard")
    task.name = "laundry"
    ...
    time_difference = date.today() + timedelta(days=2) - task.due_date
    assert abs(time_difference.total_seconds()) < 1
